...
_shelper_cmd_widget() {
    ...
    local payload
    payload=$(printf '{"input":%s,"variables":{"shell":"zsh"}}' "$escaped_input")

    # Dispatch via socat
    local response
    response=$(echo "$payload" | socat -t 60 - "UNIX-CONNECT:$sock" 2>/dev/null)
...
# Register widget and bind to Ctrl+X Ctrl+E
