...
def check_tree_sitter() -> list[DiagnosticResult]:
    ...
    if missing:
        results.append(
            DiagnosticResult("Tree-sitter grammars", False, f"Missing: {', '.join(missing)}", True)
        )
    else:
        results.append(
            DiagnosticResult("Tree-sitter grammars", True, "All supported grammars found")
        )
...
def check_sandboxes(config: Config) -> list[DiagnosticResult]:
    ...
    if sys.platform == "linux":
        ...
        if path:
            ...
            results.append(
                DiagnosticResult("Minijail sandbox", False, "minijail0 not found in PATH", True)
            )
        ...
        results.append(
            DiagnosticResult("OS Sandbox", False, f"Not supported on {sys.platform}", True)
        )
    ...
    if sys.platform == "linux":
        try:
            ...
            if abi > 0:
                results.append(
                    DiagnosticResult("Landlock (Linux)", True, f"Available (ABI v{abi})")
                )
            else:
                results.append(
                    DiagnosticResult(
                        "Landlock (Linux)", False, "Kernel does not support Landlock", True
                    )
                )
        except ImportError:
            results.append(
                DiagnosticResult("Landlock (Linux)", False, "landlock package not installed", True)
            )
    else:
