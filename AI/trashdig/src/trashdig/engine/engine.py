...
class Engine:
    ...
    def __init__(self, max_retries: int = 3, retry_delay: float = 2.0, max_context_tokens: int = 128000, compaction_threshold: float = 0.8):
    ...
    async def run(
        ...
        # Unique session for each run call
    ...
    async def _compact_history(self, agent_name: str, user_id: str, session_id: str):
        """Compact the conversation history by pruning intermediate messages.
        ...
        """
        session = await self.session_service.get_session(agent_name, user_id, session_id)
        if not session or len(session.events) <= 4:
            return

        # Prune intermediate events, keeping the first (initial prompt) 
        # and last 3 events to preserve context flow.
        # Direct modification of internal session storage as InMemorySessionService
        # doesn't provide a direct way to replace events.
        try:
            # self.session_service.sessions[app_name][user_id][session_id]
            ...
            if len(internal_session.events) > 4:
                logger.info(f"Compacting history for session {session_id}. "
                            f"Original events: {len(internal_session.events)}")
                
                # Keep first message and last 3 messages.
                new_events = [internal_session.events[0]] + internal_session.events[-3:]
                internal_session.events = new_events
