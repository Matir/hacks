...
def test_update_hypothesis_status(tmp_path):
    ...
    hypo = Hypothesis(type=TaskType.HUNT, target="app.py", description="test", confidence=0.8)
...
def test_update_hypothesis_status_delegates_to_pool(tmp_path):
    mock_db = MagicMock()
    with patch(
        "trashdig.tools.update_hypothesis_status.get_database", autospec=True, return_value=mock_db
    ):
...
def test_update_hypothesis_status_error():
    ...
    with patch(
        "trashdig.tools.update_hypothesis_status.get_database", side_effect=Exception("disk full")
    ):
