...
def mock_config():
    ...
    srv2 = McpServerConfig(
        name="server2", transport="sse", url="http://localhost:8000/sse", agents=[]
    )
...
def test_toolset_from_config_stdio():
    ...
    with (
        patch("trashdig.tools.mcp_toolsets.McpToolset"),
...
def test_toolset_from_config_sse():
    srv = McpServerConfig(name="test", transport="sse", url="http://url")
    with (
        patch("trashdig.tools.mcp_toolsets.McpToolset"),
...
def test_toolset_from_config_http():
    srv = McpServerConfig(name="test", transport="http", url="http://url")
    with (
        patch("trashdig.tools.mcp_toolsets.McpToolset"),
