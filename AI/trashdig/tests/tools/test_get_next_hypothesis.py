...
def test_get_next_hypothesis(tmp_path):
    ...
    conn.execute(
        "CREATE TABLE hypotheses (task_id TEXT, project_path TEXT, status TEXT, confidence REAL)"
    )
    conn.execute(
        "INSERT INTO hypotheses (task_id, project_path, status, confidence) VALUES ('t1', '/p', 'pending', 0.8)"
    )
    conn.execute(
        "INSERT INTO hypotheses (task_id, project_path, status, confidence) VALUES ('t2', '/p', 'pending', 0.9)"
    )
    conn.commit()
    conn.close()
