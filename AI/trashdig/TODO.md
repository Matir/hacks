...
- [ ] **[HIGH]** Attach `TrashDigCallback` to the `hunter` agent itself, not just `hunter_orchestrator` (`src/trashdig/agents/coordinator.py:217`). The wiring loop `for _agent in (*self.sub_agents, self, hunter_orchestrator)` misses `hunter` (nested via `hunter_orchestrator.sub_agents=[hunter]`), so Hunter's LLM calls — the bulk of scan work — never get cost accounting, DB logging, or `max_turns` enforcement.
- [ ] **[HIGH]** Wire `check_pause()` into `run_hunter` and `verify_finding` (`src/trashdig/agents/coordinator.py:638`, `:705`), not just `run_recon` (`:570`). README documents Space as pausing "long taint traces or hunting loops," but pressing it only flips UI state — in-flight hunting/verification loops run every remaining target to completion regardless.
- [ ] **[HIGH]** Fix shared-singleton `TrashDigCallback._turn_counts` race during parallel hunting (`src/trashdig/agents/utils/callbacks.py:77`, called from `src/trashdig/agents/coordinator.py:648`). `run_hunter` calls `reset_turn_counts()` on the process-wide singleton at its own start, but `run_full_scan` runs multiple `run_hunter` calls concurrently via `asyncio.gather` (`coordinator.py:473-486`); one segment starting wipes another's in-flight turn counts, silently disabling `max_turns` enforcement.
...
- [ ] **[MEDIUM]** Pass a real environment (at least merged with `os.environ`) into `MinijailSandbox` (`src/trashdig/sandbox/minijail.py:103`). `get_sandbox()` has no `env` parameter, so `self.env` defaults to `{}` and every Linux-sandboxed command runs with zero environment variables (no PATH/HOME/LANG), which can break env-dependent tools like semgrep/git in ways that look like unrelated failures.
- [ ] **[MEDIUM]** Close the SQLite connection in `get_next_hypothesis` (`src/trashdig/tools/get_next_hypothesis.py:20`). `with sqlite3.connect(db_path) as conn:` only commits/rolls back on exit, it doesn't close the connection — called every hypothesis-loop iteration, leaking one connection/FD each time.
- [ ] **[MEDIUM]** Fix cost-rate prefix matching to prefer the longest/most-specific match (`src/trashdig/services/cost.py:119-125`). Current `for key, val in self.rates.items(): if model_name.startswith(key)` returns the first dict-order match; e.g. `"gemini-2.0-flash"` is a prefix of the distinctly-priced `"gemini-2.0-flash-lite"`, silently mis-pricing unlisted model variants.
- [ ] **[MEDIUM]** Use `config.db_path` (not manual `config.data_dir + "/trashdig.db"`) when initializing the session service (`src/trashdig/main.py:135`). Diverges from every other DB-path consumer (`coordinator.py:154`, `services/database.py:115`, `tools/get_next_hypothesis.py:18`), so a `[database] path` override silently doesn't apply to session storage.
- [ ] **[MEDIUM]** Close the leaked pipe FD on the timeout branch in `landlock_tool.py:409-411`. The success/EOFError paths close `parent_conn` in a `finally`; the timeout branch (`child.kill(); child.join(); raise ToolTimeoutError(...)`) doesn't, leaking one FD per timed-out sandboxed tool call.
- [ ] **[LOW]** Check `cursor.rowcount` in `update_hypothesis_status` (`src/trashdig/tools/update_hypothesis_status.py:16`, backed by `services/database.py:295-304`). A stale/mistyped `task_id` updates zero rows but still returns a success message, letting the Hunter loop believe a hypothesis was closed when `get_next_hypothesis` will keep re-serving it.
- [ ] **[LOW]** Make concurrent "verify all findings" runs use `exclusive`/shared state correctly (`src/trashdig/tui/app.py:254`, `:534-558`). Each parallel `run_verification` worker sets `self._phase = "Idle"` in its own `finally`, so the status pane snaps to "Idle" as soon as the first of several concurrent verifications finishes, even though others are still running.
